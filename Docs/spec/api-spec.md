...
> `from` / `to` を指定した場合、当該時間帯に `status IN ('PENDING', 'APPROVED')` の予約が存在しないリソースのみを返す（占有中のリソースは結果から除外される）。片方のみ指定した場合は `400 Bad Request`（`code: VALIDATION_ERROR`）。ADMIN は `is_active = false` のリソースも含む。`keyword` を指定した場合は `name` または `description` にそのキーワードを含むリソースのみを返す（未指定・空白時は全件対象）。複数フィルタは AND 条件で組み合わされる。
