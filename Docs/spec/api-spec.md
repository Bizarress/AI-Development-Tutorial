...
| `keyword` | string | ❌ | `name` / `description` への大文字小文字を区別しない部分一致検索 |
| `sort` | string | ❌ | ソート順（`フィールド,方向` 形式）。フィールド: `name` / `capacity` / `createdAt`、方向: `asc` / `desc`。例: `name,asc`、`capacity,desc`。未指定またはフィールドが無効な場合は `createdAt,asc` にフォールバック |
...
> `from` / `to` を指定した場合、当該時間帯に `status IN ('PENDING', 'APPROVED')` の予約が存在しないリソースのみを返す（占有中のリソースは結果から除外される）。片方のみ指定した場合は `400 Bad Request`（`code: VALIDATION_ERROR`）。ADMIN は `is_active = false` のリソースも含む。`keyword` を指定した場合は `name` または `description` にそのキーワードを含むリソースのみを返す（未指定・空白時は全件対象）。`sort` を指定した場合は指定フィールド・方向でソートされる（`from`/`to` あり・なし両方対応）。複数フィルタは AND 条件で組み合わされる。
