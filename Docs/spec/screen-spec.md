...
| 要素 | 説明 |
|------|------|
| カテゴリフィルター | 「すべて」／「会議室」／「備品」／「社用車」のタブまたはセレクト |
| キーワード検索入力欄 | テキスト入力欄。「絞り込む」送信時に `keyword` を URL パラメータとして付与し、`name` / `description` への部分一致でリソースを絞り込む（大文字小文字区別なし）。未入力時はフィルタ解除 |
| 並び順セレクト | ドロップダウン（shadcn/ui `<Select>`）。「絞り込む」送信時に `sort=フィールド,方向` を URL パラメータとして付与。選択肢: デフォルト（登録日時昇順）／登録日時（古い順）／登録日時（新しい順）／名称（昇順）／名称（降順）／定員（少ない順）／定員（多い順）。`from`/`to` フィルタとの組み合わせにも対応 |
